...
#                            : third_interface 
#                            : fourth_interface
# Finite difference formulations used => 1st order accurate + 2nd order accurate + 3rd order accurate + 4th order accurate
...
sfi = gax_file["array8"]        # importing first intrface data here 
ffi = gax_file["array9"]        # importing second interface data here 
tfi = gax_file["array10"]       # importing third interface data here 
fti = gax_file["array11"]       # importing fourth interface data here
...
first_interface = np.array(sorted(first_interface),dtype=float)

second_interface = set(tuple(point) for point in sfi)
second_interface = np.array(sorted(second_interface),dtype=float)

third_interface = set(tuple(point) for point in tfi)
third_interface = np.array(sorted(third_interface),dtype=float)

fourth_interface = np.array(list(fti.item()), dtype=float)
...
def cord_transfer_logic_l3(a):
    x_coord=third_interface[a][0]
    y_coord=third_interface[a][1]
    # print(x_coord,"",y_coord)
    r = int(round((y_coord * conversion_factor),0))
    c = int(round((x_coord * conversion_factor),0))
    return r,c 

def cord_transfer_logic_l4(a):
    x_coord=fourth_interface[a][0]
    y_coord=fourth_interface[a][1]
    # print(x_coord,"",y_coord)
    r = int(round((y_coord * conversion_factor),0))
    c = int(round((x_coord * conversion_factor),0))
    return r,c 
...
if (RESTART == 1):
    ...
    file_path_u = r"D:/numerical computation/geometry meshing/Meshes/Time_stack_u/Time_stack_u_t033800.npz"
    file_path_v = r"D:/numerical computation/geometry meshing/Meshes/Time_stack_v/Time_stack_v_t033800.npz"
    file_path_p = r"D:/numerical computation/geometry meshing/Meshes/Time_stack_p/Time_stack_p_t033800.npz"
...
for t in range(start, total_time_steps, 1):
    ...
    for i in range(0, len(third_interface), 1):
        ip, jp = cord_transfer_logic_l3(i)
        u = u_old[ip][jp]
        v = v_old[ip][jp]
        ...
        # -------------------- Diffusion terms (2nd-order central) -------------------- #

        Hu_diffusion = (1.0/Re) * ((u_old[ip][jp+1] - 2*u_old[ip][jp] + u_old[ip][jp-1]) + (u_old[ip+1][jp] - 2*u_old[ip][jp] + u_old[ip-1][jp])) / (del_h**2)

        Hv_diffusion = (1.0/Re) * ((v_old[ip][jp+1] - 2*v_old[ip][jp] + v_old[ip][jp-1]) +(v_old[ip+1][jp] - 2*v_old[ip][jp] + v_old[ip-1][jp])) / (del_h**2)

        # -------------------- Time update -------------------- #

        Up = u_old[ip][jp] + del_t * (-Hu_conv + Hu_diffusion)
        u_p[ip][jp] = Up

        Vp = v_old[ip][jp] + del_t * (-Hv_conv + Hv_diffusion)
        v_p[ip][jp] = Vp


    for i in range(0,len(fourth_interface),1):
        ip,jp = cord_transfer_logic_l4(i)
        u = u_old[ip][jp]
        v = v_old[ip][jp]
        ...
        Hv_diffusion = (1.0/Re) * ((-v_old[ip][jp+2] + 16*v_old[ip][jp+1] - 30*v_old[ip][jp] + 16*v_old[ip][jp-1] - v_old[ip][jp-2]) + (-v_old[ip+2][jp] + 16*v_old[ip+1][jp] - 30*v_old[ip][jp] + 16*v_old[ip-1][jp] - v_old[ip-2][jp])) / (12 * del_h**2)
...
#========================================================================================================================================#
    ...
    for i in range(0, len(third_interface), 1):
        ...
        Hu_diffusion = (1.0/Re) * (
            (u_old[ib][jb+1] - 2*u_old[ib][jb] + u_old[ib][jb-1]) +
            (u_old[ib+1][jb] - 2*u_old[ib][jb] + u_old[ib-1][jb])
        ) / (del_h**2)

        Hv_diffusion = (1.0/Re) * (
            (v_old[ib][jb+1] - 2*v_old[ib][jb] + v_old[ib][jb-1]) +
            (v_old[ib+1][jb] - 2*v_old[ib][jb] + v_old[ib-1][jb])
        ) / (del_h**2)

        # ===================== Pressure gradients (2nd-order central) ===================== #

        dp_dx = (p_new[ib][jb+1] - p_new[ib][jb-1]) / (2*del_h)
        dp_dy = (p_new[ib+1][jb] - p_new[ib-1][jb]) / (2*del_h)
    ...
    for i in range(0, len(fourth_interface), 1):
        ib, jb = cord_transfer_logic_l4(i)
        u = u_old[ib][jb]
        v = v_old[ib][jb]
        ...
        #-----------------------------------------------Diffusive flux (4th order central difference)-------------------------------------------------------------------------------------------------------------------#
        # 4th-order Laplacian for u-velocity
        Hu_diffusion = (1.0/Re) * (
            (-u_old[ib][jb+2] + 16*u_old[ib][jb+1] - 30*u_old[ib][jb] + 16*u_old[ib][jb-1] - u_old[ib][jb-2]) +
            (-u_old[ib+2][jb] + 16*u_old[ib+1][jb] - 30*u_old[ib][jb] + 16*u_old[ib-1][jb] - u_old[ib-2][jb])
        ) / (12 * del_h**2)
        
        # 4th-order Laplacian for v-velocity
        Hv_diffusion = (1.0/Re) * (
            (-v_old[ib][jb+2] + 16*v_old[ib][jb+1] - 30*v_old[ib][jb] + 16*v_old[ib][jb-1] - v_old[ib][jb-2]) +
            (-v_old[ib+2][jb] + 16*v_old[ib+1][jb] - 30*v_old[ib][jb] + 16*v_old[ib-1][jb] - v_old[ib-2][jb])
        ) / (12 * del_h**2)
        
        #-----------------------------------------------------------Pressure gradient (4th order central difference)---------------------------------------------------------------------------------------------------#
        # 4th-order central difference for ∂p/∂x
        dp_dx = (-p_new[ib][jb+2] + 8*p_new[ib][jb+1] - 8*p_new[ib][jb-1] + p_new[ib][jb-2]) / (12*del_h)
        
        # 4th-order central difference for ∂p/∂y
        dp_dy = (-p_new[ib+2][jb] + 8*p_new[ib+1][jb] - 8*p_new[ib-1][jb] + p_new[ib-2][jb]) / (12*del_h)
